...
function obtener_usuarios_guardia($dia, $hora)
{
    ...
    try {
        $consulta = "select * from usuarios where id_usuario in (select usuario from horario_guardias where dia=? and hora=?)";
        $sentencia = $conexion->prepare($consulta);
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "No hay usuarios en guardia en ese momento";
...
function obtener_dias_horas_guardia($id_usuario)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    } catch (PDOException $e) {
    ...
    try {
        $consulta = "select * from horario_guardias where usuario=?";
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute([$id_usuario]);
    } catch (PDOException $e) {
        $sentencia = null;
    ...
    if ($sentencia->rowCount() > 0) {
        ...
        $respuesta["mensaje"] = "No hay horario para ese usuario";
