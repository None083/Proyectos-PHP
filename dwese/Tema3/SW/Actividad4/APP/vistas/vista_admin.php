...
if (isset($_POST["btnContNuevo"])) {
    ...
    if (!$error_cod) {
        ...
        $url = DIR_SERV_2 . "/repetido/producto/cod/" . urlencode($_POST["cod"]);
    ...
    if (!$error_nombre_corto) {
        $url = DIR_SERV_2 . "/repetido/producto/nombre_corto/" . urlencode($_POST["nombre_corto"]);
        $respuesta = consumir_servicios_REST($url, "GET");
        $json_repetido = json_decode($respuesta, true);
        if (!$json_repetido) {
            session_destroy();
    ...
    if (!$error_form) {
        ...
        $url = DIR_SERV_2 . "/producto/insertar";
...
if (isset($_POST["btnContEditar"])) {
    ...
    $error_nombre_corto = $_POST["nombre_corto"] == "";
    if (!$error_nombre_corto) {
        $url = DIR_SERV_2 . "/repetido/producto/nombre_corto/" . urlencode($_POST["nombre_corto"]) . "/cod/" . urlencode($_POST["cod"]);
        $respuesta = consumir_servicios_REST($url, "GET");
        $json_repetido = json_decode($respuesta, true);
        if (!$json_repetido) {
            session_destroy();
    ...
    if (!$error_form) {
        ...
        $url = DIR_SERV_2 . "/producto/actualizar/" . urlencode($_POST["cod"]);
...
if (isset($_POST["btnDetalles"]) || isset($_POST["btnEditar"])) {
    ...
    $url = DIR_SERV_2 . "/producto/" . urlencode($cod);
    $respuesta = consumir_servicios_REST($url, "GET");
...
if (isset($_POST["btnContBorrar"])) {
    $url = DIR_SERV_2 . "/producto/borrar/" . urlencode($_POST["btnContBorrar"]);
...
<body>
    ...
    ?>
        <form class="centrado" action="index.php" method="post">
            ...
            <p>
                <button type="submit">Volver</button> <button type="submit" name="btnContNuevo">Continuar</button>
            </p>
            </p>
        </form>
    <?php
    ...
    ?>
        <form class="centrado" action="index.php" method="post">
            ...
            <p>
                <button type="submit">Volver</button> <button type="submit" name="btnContEditar">Cambiar</button>
            </p>
            </p>
