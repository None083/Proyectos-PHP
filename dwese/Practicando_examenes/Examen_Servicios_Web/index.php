<?php
session_name("Examen_Servicios_Web");
session_start();
...
if (isset($_POST["btnLogin"])) {
    ...
    if (!$error_form_login) {
        ...
        if (!$json_login) {
            session_destroy();
            die(error_page("Gestor de Guardias", "<p>Error consumiendo el Servicio Web: <strong>" . $url . "</strong></p>"));
        }

        if (isset($json_login["error"])) {
            session_destroy();
            die(error_page("Gestor de Guardias", "<p>" . $json_login["error"] . "</p>"));
        }
