...
const PedidosModal = ({ isOpen, toggle, pedidos, usuario }) => {
  let pedidosFiltrados = null;
  if (usuario.tipo === "admin") {
    pedidosFiltrados = pedidos;
  } else {
    pedidosFiltrados = pedidos.filter(pedido => pedido.envio.nombre === usuario.nombre);
  }
          ...
          <ListGroup>
            {pedidosFiltrados.map((pedido, index) => (
              <ListGroupItem key={pedido.id}>
                ...
                <p style={{marginTop: "1rem"}}><strong>Name:</strong> {pedido.envio.nombre}</p>
