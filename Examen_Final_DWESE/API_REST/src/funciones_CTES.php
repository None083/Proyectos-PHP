...
function obtener_usuarios_guardia($dia, $hora)
{
    ...
    try {
        //obtener todos los datos de todos los usuarios que están de guardia un día a una hora

        $consulta = "select * from usuarios where id_usuario in (select usuario from horario_lectivo where dia=? and hora=? and grupo=51)";
        $sentencia = $conexion->prepare($consulta);
...
function obtener_horario($id_usuario)
{
    ...
    try {
        $consulta = "select horario_lectivo.dia, horario_lectivo.hora, grupos.nombre from horario_lectivo join grupos on horario_lectivo.grupo=grupos.id_grupo where usuario=?";
        $sentencia = $conexion->prepare($consulta);
        $sentencia->execute([$id_usuario]);
    } catch (PDOException $e) {
        $sentencia = null;
